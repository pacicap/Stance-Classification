...
# Load test dataset
test_dataset = load_jsonl_file("C:/Users/aband/OneDrive/Desktop/BTU_AI/Information_Extraction/project_3/shared_data/dataset_3_6_test_anonym.jsonl")
...
# Get prompts and completions from test dataset
prompts = [datapoint["prompt"] for datapoint in test_dataset]
completions = [datapoint["completion"] for datapoint in test_dataset]

# Convert completions to integers using label map
true_values = [LABEL_MAP[completion] for completion in completions]

# Create a list of dictionaries for each datapoint
data_list = []
for idx, prompt in enumerate(prompts):
    data_dict = {
        "prompt": prompt,
        "completion": completions[idx],
        "label_map": true_values[idx]
    }
    data_list.append(data_dict)

# Save the list to a JSON file
data_file_path = "Outputs/sentenses.json"
